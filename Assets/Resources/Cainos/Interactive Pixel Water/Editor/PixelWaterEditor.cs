...
namespace Cainos.InteractivePixelWater
{
    ...
    public class PixelWaterEditor : Cainos.LucidEditor.LucidEditor
    {
        ...
        private void OnSceneGUI()
        {
            ...
            var fmh_132_66_639140228633212374 = Quaternion.identity; Vector3 newBL = Handles.FreeMoveHandle(handlePos[0], handleSize, snap, Handles.CubeHandleCap);
            ...
            //bottom right handle
            EditorGUI.BeginChangeCheck();
            var fmh_144_66_639140228633226925 = Quaternion.identity; Vector3 newBR = Handles.FreeMoveHandle(handlePos[1], handleSize, snap, Handles.CubeHandleCap);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(water, "Resize Water");
                Undo.RecordObject(water.transform, "Move Water");
            ...
            //top left handle
            EditorGUI.BeginChangeCheck();
            var fmh_156_66_639140228633231417 = Quaternion.identity; Vector3 newTL = Handles.FreeMoveHandle(handlePos[2], handleSize, snap, Handles.CubeHandleCap);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(water, "Resize Water");
                Undo.RecordObject(water.transform, "Move Water");
            ...
            //top right
            EditorGUI.BeginChangeCheck();
            var fmh_168_66_639140228633235289 = Quaternion.identity; Vector3 newTR = Handles.FreeMoveHandle(handlePos[3], handleSize, snap, Handles.CubeHandleCap);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(water, "Resize Water");
                Undo.RecordObject(water.transform, "Move Water");
